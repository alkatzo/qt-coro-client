...
ER_HttpRequestInput::ER_HttpRequestInput() {
    initialize();
}

ER_HttpRequestInput::ER_HttpRequestInput(QString v_url_str, QString v_http_method) {
    initialize();
    url_str = v_url_str;
    http_method = v_http_method;
}

void ER_HttpRequestInput::initialize() {
    var_layout = NOT_SET;
    url_str = "";
    http_method = "GET";
}

void ER_HttpRequestInput::add_var(QString key, QString value) {
    vars[key] = value;
}

void ER_HttpRequestInput::add_file(QString variable_name, QString local_filename, QString request_filename, QString mime_type) {
    ER_HttpFileElement file;
...
ER_HttpRequestWorker::ER_HttpRequestWorker(QObject *parent, QNetworkAccessManager *_manager)
...
ER_HttpRequestWorker::~ER_HttpRequestWorker() {
...
QMap<QString, QString> ER_HttpRequestWorker::getResponseHeaders() const {
    return headers;
}

ER_HttpFileElement ER_HttpRequestWorker::getHttpFileElement(const QString &fieldname) {
    if (!files.isEmpty()) {
    ...
    return ER_HttpFileElement();
}

QByteArray *ER_HttpRequestWorker::getMultiPartField(const QString &fieldname) {
...
void ER_HttpRequestWorker::setTimeOut(int timeOutMs) {
...
void ER_HttpRequestWorker::setWorkingDirectory(const QString &path) {
    if (!path.isEmpty()) {
        workingDirectory = path;
    }
}

void ER_HttpRequestWorker::setResponseCompressionEnabled(bool enable) {
    isResponseCompressionEnabled = enable;
}

void ER_HttpRequestWorker::setRequestCompressionEnabled(bool enable) {
    isRequestCompressionEnabled = enable;
}

int  ER_HttpRequestWorker::getHttpResponseCode() const{
    return httpResponseCode;
}

QString ER_HttpRequestWorker::http_attribute_encode(QString attribute_name, QString input) {
...
void ER_HttpRequestWorker::execute(ER_HttpRequestInput *input) {
    ...
    if (input->var_layout == ADDRESS || input->var_layout == URL_ENCODED) {
        ...
        // add files
        for (QList<ER_HttpFileElement>::iterator file_info = input->files.begin(); file_info != input->files.end(); file_info++) {
    ...
    if (input->request_body.size() > 0) {
        ...
        if(!isFormData && (input->var_layout != MULTIPART) && isRequestCompressionEnabled){
            request_content.append(compress(input->request_body, 7, ER_CompressionType::Gzip));
        } else {
    ...
    if (ER_HttpRequestWorker::sslDefaultConfiguration != nullptr) {
        request.setSslConfiguration(*ER_HttpRequestWorker::sslDefaultConfiguration);
    }
...
void ER_HttpRequestWorker::on_reply_finished(QNetworkReply *reply) {
    bool codeSts = false;
...
void ER_HttpRequestWorker::on_reply_timeout(QNetworkReply *reply) {
...
void ER_HttpRequestWorker::process_response(QNetworkReply *reply) {
...
QByteArray ER_HttpRequestWorker::decompress(const QByteArray& data){
    QByteArray result;
    bool sts = false;
    do{
        z_stream strm{};
...
QByteArray ER_HttpRequestWorker::compress(const QByteArray& input, int level, ER_CompressionType compressType) {
    QByteArray output;
    static const int GZIP_WINDOW_BIT = 15+16;
    static const int ZLIB_WINDOW_BIT = 15;
    static const int CHUNK_SIZE = 8*1024;
    int windowBits;
    if( compressType == ER_CompressionType::Gzip ){
        windowBits = GZIP_WINDOW_BIT;
    } else if ( compressType == ER_CompressionType::Zlib ){
...
QSslConfiguration *ER_HttpRequestWorker::sslDefaultConfiguration;
